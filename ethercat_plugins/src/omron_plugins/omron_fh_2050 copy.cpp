...
public:
    ...
    virtual void processData(size_t index, uint8_t* domain_address){
        ...
        switch(index){
            case 0:
                ...
                break;
            case 1:
                state_interface_ptr_->at(sii_io_[index]) = EC_READ_S32(domain_address);
                break;
            case 2:
                state_interface_ptr_->at(sii_io_[index]) = EC_READ_S32(domain_address);
                break;
    ...
    virtual bool setupSlave(
        ...
        for(auto index = 0ul; index < 5; index++){
            if(paramters_.find("io."+std::to_string(index))!= paramters_.end()){
                if(paramters_.find("state_interface/"+paramters_["io."+std::to_string(index)]) != paramters_.end())
                    sii_io_[index] = std::stoi(paramters_["state_interface/"+paramters_["io."+std::to_string(index)]]);
            }
        }
        return true;
    }
